...
object contenedor {
  ...
  method peso()= 100 + self.pesosDeCosasContenidas()
  method pesosDeCosasContenidas() = cosasContenidas.sum({c => c.peso()})
  method nivelDePeligrosidad () = if (!cosasContenidas.isEmpty()) self.cosasContenidaMasPeligrosa().nivelDePeligrosidad() else 0
  method cosasContenidaMasPeligrosa() = cosasContenidas.max({c => c.nivelDePeligrosidad()})

  method meterElemento(listaDeElementos) {cosasContenidas.addAll(listaDeElementos)}
  method consecuenciaDeLaCarga() {
    cosasContenidas.fotEach({c => c.consecuenciaDeLaCarga()})
  }
}
